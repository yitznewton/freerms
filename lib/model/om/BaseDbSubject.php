...
/**
 * Base class that represents a row from the 'db_subjects' table.
 *
 * 
 *
 ...
 */
abstract class BaseDbSubject extends BaseObject  implements Persistent {


  const PEER = 'DbSubjectPeer';
	...
	/**
	 * Initializes internal state of BaseDbSubject object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     DbSubject The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = DbSubjectPeer::ID;
		}
	...
	public function setLabel($v)
	{
		...
		if ($this->label !== $v) {
			$this->label = $v;
			$this->modifiedColumns[] = DbSubjectPeer::LABEL;
		}
	...
	public function setSlug($v)
	{
		...
		if ($this->slug !== $v) {
			$this->slug = $v;
			$this->modifiedColumns[] = DbSubjectPeer::SLUG;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = DbSubjectPeer::NUM_COLUMNS - DbSubjectPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating DbSubject object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = DbSubjectPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseDbSubject:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			DbSubjectPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseDbSubject:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseDbSubject:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseDbSubject:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			DbSubjectPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = DbSubjectPeer::doInsert($this, $con);
					...
					$affectedRows += DbSubjectPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = DbSubjectPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = DbSubjectPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = DbSubjectPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(DbSubjectPeer::ID)) $criteria->add(DbSubjectPeer::ID, $this->id);
		if ($this->isColumnModified(DbSubjectPeer::LABEL)) $criteria->add(DbSubjectPeer::LABEL, $this->label);
		if ($this->isColumnModified(DbSubjectPeer::SLUG)) $criteria->add(DbSubjectPeer::SLUG, $this->slug);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(DbSubjectPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of DbSubject (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     DbSubject Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     DbSubjectPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new DbSubjectPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this DbSubject has previously been saved, it will retrieve
	 * related EResourceDbSubjectAssocs from storage. If this DbSubject is new, it will return
		...
		if ($this->collEResourceDbSubjectAssocs === null) {
			if ($this->isNew()) {
			   $this->collEResourceDbSubjectAssocs = array();
				...
				$criteria->add(EResourceDbSubjectAssocPeer::DB_SUBJECT_ID, $this->id);
	...
	public function countEResourceDbSubjectAssocs(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collEResourceDbSubjectAssocs === null) {
			if ($this->isNew()) {
				...
				$count = EResourceDbSubjectAssocPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	 ...
	 * Otherwise if this DbSubject is new, it will return
	 * an empty collection; or if this DbSubject has previously
	 * been saved, it will retrieve related EResourceDbSubjectAssocs from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in DbSubject.
	 */
	public function getEResourceDbSubjectAssocsJoinEResource($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(DbSubjectPeer::DATABASE_NAME);
		}
		...
		if ($this->collEResourceDbSubjectAssocs === null) {
			if ($this->isNew()) {
				...
				$this->collEResourceDbSubjectAssocs = EResourceDbSubjectAssocPeer::doSelectJoinEResource($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseDbSubject:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseDbSubject::%s', $method));
    }
